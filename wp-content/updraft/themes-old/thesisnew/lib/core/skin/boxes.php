...
class thesis_stylesheets_link extends thesis_box {
	...
	public function html() {
		$styles = $links = array();
		$styles['skin'] = array(
			'url' => THESIS_USER_SKIN_URL . '/css.css',
...
class thesis_post_excerpt extends thesis_box {
	...
	protected function html_options() {
		...
		$html['read_more_show'] = array(
			...
			'options' => array('show' => __('Show "Read More" link and the end of an excerpt.')),
		...
		$html['read_more_text'] = array(
			...
			'parent' => array('read_more_show' => 'show'));
		unset($html['id']);
		return $html;
	}
	...
	public function html($args = array()) {
		...
		if (isset($this->options['ellipsis']['show']) || isset($this->options['read_more_show']['show'])) {
			add_filter('excerpt_more', '__return_false', 1);
		...
		echo
			...
			"$tab</div>\n";
		if (isset($this->options['ellipsis']['show']) || isset($this->options['read_more_show']['show'])) {
			remove_filter('excerpt_more', '__return_false', 1);
			remove_filter('thesis_trim_excerpt', array($this, 'more'), 100);
		}
	}
	
	public function more($in = '') {
		$out = '';
		...
		if (isset($this->options['read_more_show']['show']))
			$out .= "\n<a href=\"". get_permalink(). "\">". esc_attr((!empty($this->options['read_more_text']) ? $this->options['read_more_text'] : __('Read More', 'thesis'))). "</a>";
